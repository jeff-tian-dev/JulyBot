...
- **Fetch Link and Downloads answer with a popup modal, not an ephemeral message** (`InfoModal`). Discord has no read-only display modal, so the text sits in a prefilled paragraph `TextInput`; Discord always draws Cancel/Submit and its own "do not share passwords" banner and **neither can be removed**. `InfoModal.callback` just `defer()`s so Submit closes the dialog without saving. A modal must be the **first** response to an interaction — that's why `_on_fetch` sends it *before* refreshing the count (which edits the message, not the interaction). Permission refusals stay ephemeral messages; a popup for an error reads worse.
- Modal bodies are plain text, so `<@id>` would render as a raw id — the downloader list resolves members to display names instead.
