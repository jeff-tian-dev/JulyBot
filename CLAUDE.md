...
- **Fetch Link and Downloads answer with an ephemeral embed, NOT a modal** (`link_embed` / `downloaders_embed`). A modal was tried on 2026-08-10 and reverted: a modal body can only hold a `TextInput`, which is **always editable**, so the link and the stats text were editable by the viewer and the URL rendered as raw text in a box instead of a clickable link. Discord also forces Cancel/Submit and a "do not share passwords" banner onto every modal. Only a message renders a real hyperlink and is read-only. Modals remain correct for **Edit**, which is a genuine form.
