...
- The embed body is composed by `build_base_body()` into the in-game format: `## title`, a `------------------------` divider, `🏰 **CC:**` block, then `*Notes:*` + description. Blank sections are skipped entirely. Individually-valid fields can still overflow Discord's 4096-char embed description, so the combined length is re-checked there — reachable via the modal, which takes raw text.
