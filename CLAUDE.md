...
- **A line of exactly `---` renders as a true full-width horizontal rule in Discord.** The original literal `------------------------` was a pre-`<hr>` workaround that renders as a ragged fixed-width string not matching the embed width. Don't "fix" `---` back into a run of dashes.
