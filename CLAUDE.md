...
- The embed body is composed by `build_base_body()` into the in-game format: `## title`, a `------------------------` divider, a `**CC:**` block, then the description **verbatim with no heading of its own** — a poster who wants a "Notes:" label types it into the description themselves. Don't reintroduce an injected heading or decorative emoji on the CC line. Blank sections are skipped entirely. Individually-valid fields can still overflow Discord's 4096-char embed description, so the combined length is re-checked there — reachable via the modal, which takes raw text.
