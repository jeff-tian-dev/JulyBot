...
from modules.twitter_monitor import embeds as embeds_mod
...
def _role_ping_content(role_id: int | None = None) -> str | None:
    ...
    rid = settings.TWITTER_PING_ROLE_ID if role_id is None else role_id
    if rid:
        return f"<@&{rid}>"
    return None
...
async def poll_twitter_accounts(pool: asyncpg.Pool, bot: disnake.Client) -> dict:
    """Poll all watched accounts and post new tweets to configured Discord channels."""
    ...
    if not bot.is_ready():
        logger.debug("Discord bot not ready; skipping twitter poll")
        return summary

    try:
        accounts = await storage.get_all_watched_accounts(pool)
    except Exception:
        logger.exception("Failed to fetch watched twitter accounts")
        summary["errors"] += 1
        return summary
    ...
    try:
        app = await twitter_client.get_client()
    except Exception as exc:
        if twitter_client.is_auth_error(exc):
            logger.error("Twitter auth failed; check TWITTER_COOKIES")
        else:
            logger.exception("Failed to connect twitter client")
        summary["errors"] += 1
        return summary
    ...
    for account in accounts:
        ...
        channel_id = account.get("twitter_channel_id")
        enabled = account.get("twitter_enabled")
        ...
        if channel is None:
            logger.warning(
                "twitter_channel_id=%s not found for guild_id=%s",
                channel_id,
                guild_id,
            )
            summary["errors"] += 1
        ...
        try:
            ...
            logger.warning("Twitter rate limit hit; sleeping %ss", wait)
        ...
        for tweet in candidates:
            ...
            try:
                ...
                await channel.send(
                    content=_role_ping_content(),
                    embeds=tweet_embeds,
                    components=components,
                    allowed_mentions=_role_ping_allowed_mentions(),
                )
            except Exception:
                logger.exception(
    ...
    logger.info(
        "Twitter poll complete: accounts=%d tweets_posted=%d errors=%d",
